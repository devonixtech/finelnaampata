...
// Simple Online/Offline badge ΓÇö green when vendor is logged in, red when not
...
export default function BusinessDetailClient({
  ...
  const handleReviewSubmit = async (e?: React.FormEvent) => {
    ...
    if (!user.isPhoneVerified) {
      toast.error("Please verify your phone number before writing a review. Go to Settings ΓåÆ Phone Verification.");
      return;
    }
    if (!business) return;
                            ...
                            <div className="mt-3 space-y-2.5">
                              {business.namedPhoneNumbers.map((npn: any, idx: number) => (
                                <div key={idx} className="flex flex-col bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                                  <span className="text-[10px] text-slate-500 font-black uppercase tracking-wider mb-0.5">{npn.label} {npn.personName && `ΓÇó ${npn.personName}`}</span>
